...
```text
========================================================================================
Document ID:     WT-2026-0912-MASTER-003
...
=======================================================================
 ...
 [7] Run Full System Diagnostics & Automated Test Suite (307 Tests)
...
- **Option [1] (Start All Services)** prompts the operator:
  ...
  - `[2] Unattended Mode (Background Headless)`: Runs Chromium in headless mode via `--headless=new` with `context_headless=False`, ensuring AntiCaptcha extension loading, service worker registration, identical DOM extraction, and zero reliance on active desktop sessions. Sets `PLAYWRIGHT_HEADLESS=true`.
- **E2E Parity Verification**: Verified through `scripts/verify_attended_unattended_parity_e2e.py` and `backend/tests/test_attended_unattended_parity.py` with 100% equivalence (0 discrepancies).
...
## 7. Walkthrough 6: Dynamic Notification & Email Engine (`/notifications` & `/settings`)

Located at `/notifications` and the Email tab in `/settings`, this operational hub manages all automated notification flows, email templates, and event rules:

### 7.1 Notification Delivery History
- Comprehensive delivery log showing Event Type, Recipient, Subject, Delivery Status (`SENT`, `FAILED`, `PENDING`), Delivery Channel (`SMTP`, `SES`, `GRAPH`, `MOCK`), and Latency (ms).
- Search, filter by event type, and date range selector.
- Clickable modal to view rendered HTML delivery receipts with raw payload inspections.
...
- **Live Dynamic Preview**: Endpoint `GET /api/v1/notifications/templates/{id}/preview` renders templates in real-time with sample claim variables without sending real emails.

### 7.3 Notification Rules & Recipient Matrix
...
### 7.4 Live Interactive Test Email Sandbox
- Modal under `/settings` -> Email tab allowing operators to enter a custom recipient, subject, and body, and click "Send Test Email".
- Displays live latency benchmark, provider acknowledgement, and delivery status badge.
...
6. **Email & Notifications**: SMTP Host, Port, Username, Password/API Key, Use TLS, From Address, Provider Type (`smtp`, `ses`, `graph`, `mock`), live "Test Connection" button, and interactive "Send Test Email" sandbox.
...
========================================================================
 1. Pytest Backend Suite:       280 / 280 PASSED across 28 suites (100%)
 ...
 6. PowerShell AST Syntax:      0 ERRORS (setup_local.ps1 & utilities)
