...
```text
========================================================================================
Document ID:     WT-2026-0905-MASTER-001
...
Source Audit:    walkthrough.md through walkthrough_7.md + walkthrough_anticaptcha*
Code Base Truth: Verified against live Next.js App Router & FastAPI/Celery Backend
========================================================================================
```
...
sequenceDiagram
    ...
    Operator->>UI: Trigger Automation (Single or Bulk)
    UI->>API: POST /api/v1/claims/{id}/start
    API->>Celery: Enqueue orchestrate_court_scrapers_task
    Celery->>Browser: Launch Multi-Tab Chrome Session (Attended/Headless)
    Browser->>Portals: Navigate 8 Portals (Florida / Texas State Routing)
    Note over Browser,Portals: AntiCaptcha & Turnstile Solver Active
...
=======================================================================
 ...
 [7] Run Full System Diagnostics & Automated Test Suite (157 Tests)
...
### 2.2 Execution Modes: Attended GUI vs. Unattended Headless
- **Option [1] (Start All Services)** prompts the operator:
  - `[1] Attended Mode (Recommended for Live Monitoring)`: Launches a visible Google Chrome or Microsoft Edge browser window. Operators can watch the bot type, navigate county portals, and solve CAPTCHAs live on screen. Sets `PLAYWRIGHT_HEADLESS=false`.
  - `[2] Unattended Mode (Background Automation)`: Runs browsers silently in headless mode for high-throughput automated execution. Sets `PLAYWRIGHT_HEADLESS=true`.
...
- **Filter Preset Manager (`<FilterPresetManager />` §83)**:
...
- **Retry Failed Portals Only (§66)**: Prominent amber action button in the header and card grid that re-executes strictly the failed portals, preserving cases from already-successful portals.
...
### 5.4 Scraped Public Court Cases Section (P5 Requirement)
...
### 5.7 Claim Audit Trail & Provenance Timeline (§73)
...
## 7. Walkthrough 6: Fuzzy Match Exceptions Review (`/exceptions`)
...
## 8. Walkthrough 7: RPA & Operational Health Console (`/health`)
...
- **RPA Browser Automation Health Panel (§62)**:
...
## 9. Walkthrough 8: Enterprise Audit Trail Console (`/audit`)
...
## 10. Walkthrough 9: Solution-Wide Brand & Identity Console (`/branding`)
...
## 11. Walkthrough 10: Automation & Robot Configuration (`/settings`)

Located at `/settings`, dedicated strictly to robot automation and integration parameters across 5 clean tabs:
...
## 12. Automated Verification & Quality Assurance Baseline
...
========================================================================
 1. Pytest Backend Suite:       157 / 157 PASSED (100% Pass Rate)
 2. Ruff Python Linter:         0 ERRORS (app & tests clean)
 3. TypeScript Compiler:        0 ERRORS (npx tsc --noEmit)
 4. Next.js Production Build:   11 / 11 PAGES COMPILED (100% Clean)
 5. PowerShell AST Syntax:      0 ERRORS (setup.ps1 & setup_local.ps1)
...
## 13. Verification & Governance Statement
