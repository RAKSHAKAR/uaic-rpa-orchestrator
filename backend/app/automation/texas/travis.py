...
logger = logging.getLogger("uaic_orchestrator.automation.travis")


class TravisScraper(BaseCourtScraper):
    """Scraper for Travis County Odyssey Portal."""

    def __init__(self, base_url: str | None = None, **kwargs):
        super().__init__(
            county_name="Travis County (TX)",
            base_url=base_url or "https://odysseyweb.traviscountytx.gov/Portal/",
            **kwargs,
        )
    ...
    async def search_by_party_name(
        ...
        **kwargs,
        ...
        if "Dashboard/29" not in nav_url:
            nav_url = f"{nav_url.rstrip('/')}/Home/Dashboard/29"
        ...
        # 1. Fill Smart Search criteria: LastName,FirstName (Travis does NOT use DOL)
